...
class Database:
    ...
    def Get_Vectors_AuC(self, auc_id, action, **kwargs):
        ...
        if action == "air":
            ...
            vect = S6a_crypt.generate_2g3g_vector(key_data['ki'], key_data['opc'], key_data['amf'],
                                                  int(key_data['sqn']), int(key_data['algo']))
            vector_list = []
    ...
    def update_gsup(self, imsi: str, role: IPAPeerRole, new_id: Optional[str]) -> str:
        self.logTool.log(service='Database', level='debug',
                         message=f"Updating GSUP record {role.name} for IMSI: {imsi} with new ID: {new_id}",
