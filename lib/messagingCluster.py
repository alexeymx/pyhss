...
class RedisMessaging:
    ...
    def __init__(self, host: str='localhost', port: int=6379, useUnixSocket: bool=False, unixSocketPath: str='/var/run/redis/redis-server.sock', useCluster=True):
        if useUnixSocket:
    ...
    def awaitBulkMessage(self, key: str, count: int = 100, usePrefix: bool = False, prefixHostname: str = 'unknown',
                         prefixServiceName: str = 'common'):
        """
        Blocks until messages are available at the given key (single-shard in Redis Cluster).
        Since BLMPOP is not cluster-compatible, use BRPOP in a cluster-friendly way.
        """
        try:
            key = self.handlePrefix(key=key, usePrefix=usePrefix, prefixHostname=prefixHostname,
                                    prefixServiceName=prefixServiceName)

            messages = []
            for _ in range(count):
                message = self.redisClient.brpop(key, timeout=0)  # Block until a message is available
                if not message:
                    break
